...
draw_plugin_chart(void)
	...
	gkrellm_draw_chart_text(chart, plugin_style_id, "\\f50\\b\\f30");
	...
	if (pGK->second_tick)
		...
		*/
		if (plugin_enable)
			...
			{
			temp = 35;
			gkrellm_set_chartconfig_fixed_grids(chart_config, 3);
			}
		t = temp - temp_min;
		if (t < 0)
			t = 0;
printf("store temperature %d\n", temp);
		gkrellm_store_chartdata(chart, 0, t);
...
create_plugin(GtkWidget *vbox, gint first_create)
	...
	*/
	gkrellm_set_chart_height_default(chart, 30);
	...
	*/
	plugin_cd = gkrellm_add_default_chartdata(chart, "Temperature Data");
	...
	*/
	...
	gkrellm_set_chartdata_flags(plugin_cd,
				CHARTDATA_NO_CONFIG_INVERTED | CHARTDATA_NO_CONFIG_DRAW_STYLE);
	...
	*/
	...
	gkrellm_set_chartconfig_flags(chart_config,
			NO_CONFIG_AUTO_GRID_RESOLUTION | NO_CONFIG_FIXED_GRIDS);
	gkrellm_set_chartconfig_auto_grid_resolution(chart_config, FALSE);

	/* set up 2 grids and 10 degrees/grid so will have +-10 degress
	*/
	gkrellm_set_chartconfig_fixed_grids(chart_config, 2);
	gkrellm_set_chartconfig_grid_resolution(chart_config, 10);

	/* Setup temperature limits
	*/
	temp_min = 30;		/* so center will be 40 and max will be 50 */
